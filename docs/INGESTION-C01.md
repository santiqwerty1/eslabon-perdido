...
`delta.py` sólo aplica un delta sobre la revisión de la que parte y sólo revierte el último aplicado según el historial; fuera de ese orden se niega y dice cuál va antes, también con `--dry-run`. Sólo trabaja con deltas de `knowledge/deltas/`, y no revierte uno que haya cambiado desde que se aplicó: el historial guarda su hash. Revertir la sección con su conversión aplicada borraría las menciones que la conversión actualizó y dejaría sus registros sin procedencia.
